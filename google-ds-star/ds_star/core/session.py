...
class DSStarSession:
    ...
    async def run_with_state(
        ...
        # Return state (it's been updated in place)
        return DSStarState(
            query=query,
            data_files=self._expand_file_paths(data_files),
            # Note: This creates a new state - in production you'd track the original
        )
